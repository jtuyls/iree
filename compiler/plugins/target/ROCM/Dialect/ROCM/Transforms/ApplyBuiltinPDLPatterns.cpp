...
public:
  ...
  void runOnOperation() override {
    ...
    SmallVector<IREE::Util::FuncOp> ukernelFunctions;
    auto res = moduleOp.walk([&](Operation *op) {
      auto builtinName =
          dyn_cast_or_null<StringAttr>(op->getAttr(kBuiltinName));
      auto ukernelDesc = dyn_cast_or_null<IREE::Codegen::UKernelDescriptorAttr>(
          op->getAttr(kUKernelDescriptorName));
      ...
      auto funcOp = dyn_cast_if_present<IREE::Util::FuncOp>(
    ...
    for (IREE::Util::FuncOp funcOp : ukernelFunctions) {
