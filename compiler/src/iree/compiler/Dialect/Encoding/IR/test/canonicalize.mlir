...
// Test that iree_encoding.dim folds when encoding_dims are constants.
// The folder traces through the producer chain and folds constant values.

#encoding = #iree_encoding.testing<>
func.func @dim_fold_constant(%arg0: tensor<?x?xf32>) -> (index, index, index) {
...
// CHECK-LABEL: func.func @dim_fold_constant
...
// Test that iree_encoding.dim folds through tensor.cast when encoding_dims are constants.

#encoding = #iree_encoding.testing<>
func.func @dim_fold_through_cast(%arg0: tensor<?x?xf32>) -> (index, index, index) {
  %c128 = arith.constant 128 : index
  %c256 = arith.constant 256 : index
  %c64 = arith.constant 64 : index
  %0 = iree_encoding.set_encoding %arg0 encoding_dims{%c128, %c256, %c64} : tensor<?x?xf32> -> tensor<?x?xf32, #encoding>
  ...
  %dim_k = iree_encoding.dim %1[2] : tensor<4x8xf32, #encoding>
  return %dim_m, %dim_n, %dim_k : index, index, index
}
// CHECK-LABEL: func.func @dim_fold_through_cast
// CHECK-DAG:     %[[C128:.+]] = arith.constant 128 : index
// CHECK-DAG:     %[[C256:.+]] = arith.constant 256 : index
// CHECK-DAG:     %[[C64:.+]] = arith.constant 64 : index
// CHECK:         return %[[C128]], %[[C256]], %[[C64]]
...
// Test that canonicalization resolves iree_encoding.dim to the dynamic values
// from set_encoding.

#encoding = #iree_encoding.testing<>
func.func @dim_canonicalize_dynamic(%arg0: tensor<?x?xf32>, %m: index, %n: index, %k: index) -> (index, index, index) {
...
// CHECK-LABEL: func.func @dim_canonicalize_dynamic
...
#encoding = #iree_encoding.testing<>
func.func @dim_canonicalize_through_cast(%arg0: tensor<?x?xf32>, %m: index, %n: index) -> (index, index) {
...
// CHECK-LABEL: func.func @dim_canonicalize_through_cast
// CHECK-SAME:    %[[ARG0:[a-zA-Z0-9]+]]: tensor<?x?xf32>
// CHECK-SAME:    %[[M:[a-zA-Z0-9]+]]: index
// CHECK-SAME:    %[[N:[a-zA-Z0-9]+]]: index
// CHECK:         return %[[M]], %[[N]]
...
// Test that iree_encoding.dim is NOT resolved when there's no set_encoding in producer chain.

#encoding = #iree_encoding.testing<>
func.func @dim_no_set_encoding(%arg0: tensor<?x?xf32, #encoding>) -> index {
  %dim = iree_encoding.dim %arg0[0] : tensor<?x?xf32, #encoding>
  return %dim : index
}
// CHECK-LABEL: func.func @dim_no_set_encoding
