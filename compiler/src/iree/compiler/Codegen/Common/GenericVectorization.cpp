...
getVectorSizes(Operation *op, bool useConfiguredVectorSizes) {
  ...
  if (useConfiguredVectorSizes && loweringConfig) {
    LLVM_DEBUG(llvm::dbgs() << "useConfiguredVectorSizes && loweringConfig\n");
...
void GenericVectorizationPass::runOnOperation() {
  ...
  for (Operation *op : candidates) {
    ...
    LLVM_DEBUG(llvm::dbgs() << "enableVectorMasking: " << enableVectorMasking << "\n");
    if (enableVectorMasking) {
      ...
      if (vectorSizesAndScalableDims) {
        ...
        LLVM_DEBUG(llvm::dbgs() << "vectorSizes: " << llvm::interleaved_array(vectorSizes) << "\n");
        LLVM_DEBUG(llvm::dbgs() << "scalableVecDims: " << llvm::interleaved_array(scalableVecDims) << "\n");
      }
    }
